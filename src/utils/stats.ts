...
const AGREE_ADDRESS = '0xf467C7d5a4A9C4687fFc7986aC6aD5A4c81E1404'
const PARYS_ADDRESS = '0x10398AbC267496E49106B07dd6BE13364D10dC71'
...
export const kiteTokenStats = async () => {
    ...
    const kiteERC20 = new ethers.Contract(AGREE_ADDRESS, ERC20_ABI, provider)
    ...
    for (let i = 1; i <= vestingPlans; i++) {
        ...
        if (plan.token.toLowerCase() == AGREE_ADDRESS.toLowerCase()) {
...
export const haiTokenStats = async () => {
    const provider = new ethers.providers.JsonRpcProvider(process.env.VITE_MAINNET_PUBLIC_RPC)
    const haiERC20 = new ethers.Contract(PARYS_ADDRESS, ERC20_ABI, provider)
